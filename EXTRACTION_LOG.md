...
3. `php artisan test`: **73 lulus, 2 todo, 0 gagal** (225 assertion; termasuk `VendorPageTest` dan tes captcha baru). `vendor/bin/pint --test`: passed.
...
- `composer.json`: `name` (`ciptadigital/alh-core`), `description`, `license` (`proprietary`), sesuai default yang boleh diubah manusia.
