...
new #[Layout('layouts.app')] #[Title('Daftar Mitra')] class extends Component {
    use WithPagination;
    ...
    {
        abort_unless(auth()->user()?->can('partner:partner-view'), 403);
    }
    ...
    /**
     * The account number is encrypted, so it cannot be searched (and is not selected by any
     * search term): matching on it would also reveal that a number exists.
     */
    #[Computed]
    ...
    #[On('partner-deactivated')]
    public function refreshPartnersAfterDeactivate()
    {
        unset($this->partners);
    }
};
...
<div class="">
    ...
    <div class="rounded-2xl border border-neutral-border bg-neutral-card p-5 xl:p-8">
      ...
      <div class="mb-6 flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p class="mt-1 text-sm text-neutral-label">
            Kelola mitra yang membeli barang di toko, beserta kontak dan data pembayarannya
          </p>
        </div>

        <div class="flex items-center gap-3">
          <button wire:click="$dispatch('open-partner-form')"
      ...
      <div class="mb-6 space-y-4">
        <div class="relative w-full max-w-sm">
          ...
          <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari nama, telepon, kota, area..."
          ...
          <table class="w-full min-w-[1000px] font-sans">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
              @forelse ($this->partners as $partner)
                <tr wire:key="partner-{{ $partner->id }}" class="transition-colors hover:bg-gray-50/30 dark:hover:bg-white/[0.01]">
                  <td class="whitespace-nowrap px-6 py-4">
                    <span class="text-sm font-semibold text-neutral-heading dark:text-white/90">#{{ $partner->id }}</span>
                  </td>
                  <td class="whitespace-nowrap px-6 py-4">
                    <span class="block text-sm font-semibold text-gray-800 dark:text-white/90">{{ $partner->name }}</span>
                    @if ($partner->notes)
                      <span class="block max-w-xs truncate text-xs text-gray-400">{{ $partner->notes }}</span>
                    @endif
                  </td>
                  <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-600 dark:text-gray-300">
                    <span class="block">{{ $partner->city }}</span>
                    @if ($partner->area)
                      <span class="block text-xs text-gray-400">{{ $partner->area }}</span>
                    @endif
                  </td>
                  <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-600 dark:text-gray-300">
                    {{ $partner->phone }}
                  ...
                  <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-600 dark:text-gray-300">
                    @if ($partner->payment_method)
                      <span class="block">{{ $partner->paymentMethodLabel() }}@if ($partner->payment_provider) · {{ $partner->payment_provider }}@endif</span>
                      <span class="block text-xs text-gray-400">
                        @if ($partner->account_name){{ $partner->account_name }} · @endif
                        @if ($partner->accountNumberIsUnreadable())
                          Nomor tidak terbaca
                        @else
                  ...
                  <td class="whitespace-nowrap px-6 py-4 text-center">
                    <div class="flex items-center justify-center gap-3">
                      <button wire:click="$dispatch('open-partner-form', { id: {{ $partner->id }} })"
                      ...
                      @if ($partner->status === 'active')
                        <button wire:click="$dispatch('open-partner-deactivate', { id: {{ $partner->id }}, name: '{{ addslashes($partner->name) }}' })"
      ...
      <div class="flex flex-col items-center justify-between gap-4 border-t border-gray-100 pt-6 md:flex-row dark:border-gray-800">
        ...
        @php $paginator = $this->partners; @endphp
  ...
  <livewire:partner.partner-form-modal />
  <livewire:partner.partner-deactivate-modal />
</div>
