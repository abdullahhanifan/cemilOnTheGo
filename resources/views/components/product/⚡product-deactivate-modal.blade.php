...
new class extends Component {
     ...
     * see the note in product-form-modal.blade.php.
     *
     * Products are deactivated through `status`, never hard-deleted, so their
     * history stays intact and they can be reactivated from the edit form.
     */
    public function deactivateProduct(): void
    {
        abort_unless(auth()->user()?->can('product:product-delete'), 403);
        ...
        try {
            ...
            report($e);
            $this->isDeactivateOpen = false;
            $msg = 'Gagal menonaktifkan produk. Silakan coba lagi.';
    ...
    <div
      ...
      <div class="pb-4 text-center">
        ...
        <p class="mt-2 text-xs text-neutral-label dark:text-gray-400">
          Produk ini akan ditandai nonaktif. Datanya tetap tersimpan dan bisa diaktifkan kembali lewat form edit.
        </p>
        @if ($productName)
          <div class="mt-3 rounded-xl border border-gray-100 bg-gray-50 p-3 dark:border-gray-800 dark:bg-white/[0.02]">
            <span class="block text-sm font-semibold text-gray-800 dark:text-white">{{ $productName }}</span>
          </div>
        @endif
      </div>
      ...
      <div class="mt-4 flex items-center gap-3">
        ...
        <button type="button" wire:click="deactivateProduct" wire:loading.attr="disabled"
          ...
          <x-ui.spinner.spinner-four :icon-only="true" class="w-5 h-5 text-white" wire:loading wire:target="deactivateProduct" />
